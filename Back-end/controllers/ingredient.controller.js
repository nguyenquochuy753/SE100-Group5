...
const ingredientController = {
    ...
    getAllIngredients: async (req, res) => {
        try {
            const allIngredient = await ingredientModel.find({}).populate({path:"ma_loai_nguyen_lieu",select:"ten_loai_nguyen_lieu"});
    ...
    updateIngredient: async (req, res) => {
        ...
        const formattedTenNguyenLieu = lowercaseTenNguyenLieu.charAt(0).toUpperCase() + lowercaseTenNguyenLieu.slice(1);
        try {
            await ingredientModel.findByIdAndUpdate(id,{ten_nguyen_lieu : formattedTenNguyenLieu , khoi_luong_ton : khoi_luong_ton , ma_loai_nguyen_lieu : ma_loai_nguyen_lieu},{new:true});
