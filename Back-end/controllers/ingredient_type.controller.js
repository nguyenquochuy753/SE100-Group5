...
const ingredient_typeController = {
  addIngredient_type: async (req, res) => {
    ...
    const lowercaseTenLoaiNguyenLieu = ten_loai_nguyen_lieu
      .trim()
      .toLowerCase();
    const formattedTenLoaiNguyenLieu =
      lowercaseTenLoaiNguyenLieu.charAt(0).toUpperCase() +
      lowercaseTenLoaiNguyenLieu.slice(1);
  ...
  getIngredientTypeById: async (req, res) => {
    const id = req.params.id;
    try {
      const ingredientType = await ingredient_typeModel.findById(id );
  ...
  updateIngredientType: async (req, res) => {
    ...
    const formattedTenLoaiNguyenLieu =
      ...
      lowercaseTenLoaiNguyenLieu.slice(1);
    try {
      await ingredient_typeModel.findByIdAndUpdate(
        id,
        { ten_loai_nguyen_lieu: formattedTenLoaiNguyenLieu },
        { new: true }
      );
