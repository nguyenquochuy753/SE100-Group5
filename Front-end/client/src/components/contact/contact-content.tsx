...
const ContactBox: React.FC<{ items?: any }> = ({items: {data}, setTenKhachHang, setSdt}) => {
    const {t} = useTranslation('common');
    let [contactData, setContactData] = useState(data);
    const {openModal} = useModalAction();
    const handleAddContact = (name, phone) => {
        setContactData(preData => [...preData, {
            "id": 1,
            "title": name,
            "default": true,
            "number": phone
    ...
    function handlePopupView(item: any) {
        openModal('PHONE_NUMBER', handleAddContact);
    ...
    const [selected, setSelected] = useState(contactData[0]);
    return (
        <>
                ...
                >
                    ...
                    {contactData?.map((item: any, index: any) => (
                        <RadioGroup.Option
                            ...
                            className={({active, checked}) =>
                                `${item.default ? 'border-skin-primary' : 'border-skin-base'}
                  ...
                  border-2 relative shadow-md focus:outline-none rounded p-5 block cursor-pointer min-h-[112px] h-full group address__box`
                        ...
                        >
                            ...
                            <div
                                className="flex absolute end-3 top-3 z-30 lg:opacity-0 transition-all address__actions">
                                <button
